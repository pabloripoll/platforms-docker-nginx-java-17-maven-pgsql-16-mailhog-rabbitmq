...
- [Linux Oracle 9](https://hub.docker.com/_/oraclelinux) *(Optional Dockerfile)*
- Java 17
- Maven
<br><br>

It can be installed the most known **JAVA** frameworks:
...
COMPOSE_PROJECT_IMGK="alpine-3.24-nginx-java-17"        # <- real main image keys to manage automations for sharing resources -------------------------------> #
...
To automatically run the JAVA application, create the Supervisord service that runs the application. You can choose the dev or production version, and set it according to your project requirements

- `./docker/config/supervisor/conf.d-sample/java.conf` -> `./docker/config/supervisor/conf.d/java.conf`
...
`./platform/nginx-java-17/docker/config/supervisor/conf.d-sample`

Before building the container you must copy that directory to:
`./platform/nginx-java-17/docker/config/supervisor/conf.d`

Make sure the copied conf.d contains at least the service files needed to run Nginx and Java (for example, supervisor program entries for both).

```bash
$ cd ./platform/nginx-java-17/docker/config/supervisor
$ cp -vn conf.d-sample/nginx.conf conf.d-sample/java.conf conf.d/
'conf.d-sample/nginx.conf' -> 'conf.d/nginx.conf'
'conf.d-sample/java.conf' -> 'conf.d/java.conf'
```

This approach lets developers run additional worker processes locally without changing the shared platform settings. If you need to update Nginx, Java, or supervisord configurations on a running container, there are Makefile recipes in `./platform/nginx-java-17/Makefile` that can apply changes *(reload or update services)* without destroying and rebuilding the container. Check the Makefile for available targets and usage by executing `$ make help` in its directory.
<br><br>
